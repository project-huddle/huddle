...
export async function body(request: Request): Promise<Record<string, unknown> | null> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > config.maxJsonBytes) return null;
  try {
    const text = await request.text();
    if (new TextEncoder().encode(text).byteLength > config.maxJsonBytes) return null;
