...
export function createRevokeUnauthorizedSocketAccess(
  channelAccess: ChannelAccess,
) {
  return async function revokeUnauthorizedSocketAccess(
    userId: string,
  ): Promise<void> {
    for (const socket of socketsByUser.get(userId) ?? []) {
      const channelId = session(socket).channelId;
      if (!channelId || (await channelAccess.channelForUser(userId, channelId)))
        continue;
      leaveCall(socket);
...
export function createRealtimeWebSocket(
  ...
  return {
    async open(ws: RealtimeSocket) {
      ...
      current.channelId =
        (await channelAccess.firstChannelForUser(user.id))?.id ?? null;
    ...
    async message(ws: RealtimeSocket, raw: unknown) {
      ...
      if (event.type === "chat_message") {
        const channelId =
          ...
          (await channelAccess.firstChannelForUser(session(ws).user.id))?.id ||
          "";
        if (
          !(await channelAccess.channelForUser(session(ws).user.id, channelId))
        )
          return send(ws, {
            type: "error",
            code: "FORBIDDEN",
        ...
        const reply =
          typeof event.replyToId === "string"
            ? await messageActions.messageForUser(
                session(ws).user.id,
                event.replyToId,
              )
            : null;
        const replyToId = reply?.channelId === channelId ? reply.id : null;
        const message = await messageActions.saveMessage(
          session(ws).user,
          channelId,
          content,
          media,
      ...
      if (
        ...
        const target = messageId
          ? await messageActions.messageForUser(session(ws).user.id, messageId)
          : null;
        ...
        if (event.type === "edit_message") {
          ...
          message = await messageActions.editMessage(
            session(ws).user.id,
            messageId,
            content,
          );
        } else if (event.type === "delete_message")
          message = await messageActions.deleteMessage(
            session(ws).user.id,
            messageId,
          );
        else
          message =
            typeof event.emoji === "string"
              ? await messageActions.reactMessage(
                  session(ws).user.id,
                  messageId,
                  event.emoji,
                )
      ...
      if (
        ...
        if (event.type === "ice_candidate")
          forwarded.candidate = event.candidate;
