...
export function createAuthRoutes(identityService: IdentityService) {
  const { sendIdentityCode, verifyTwoFactorChallenge } = identityService;
  return new Elysia({ name: "auth-routes" })
    ...
    .post(
      ...
      async ({ body: input }) => {
        ...
        const verified = await userForId(userId);
        if (!verified) return error(404, "NOT_FOUND", "User not found.");
        return json({
          user: verified,
