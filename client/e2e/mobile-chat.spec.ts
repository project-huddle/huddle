...
async function register(request: APIRequestContext, prefix: string) {
  const account = { email: `${prefix}-${Date.now()}@example.com`, displayName: prefix, password: "secure-password" }
  const response = await request.post(`${apiOrigin}/auth/register`, { data: account })
...
test.describe("mobile chat", () => {
  ...
  test("opens navigation drawer and changes channel", async ({ page, request }) => {
    const account = await register(request, "mobile-user")
    await login(page, account)
    await expect(page.getByText(/Minha comunidade · #/)).toBeVisible()

    await page.getByRole("button", { name: "Abrir navegação" }).click()
    await expect(page.getByRole("dialog", { name: "Navegação do servidor" })).toBeVisible()
