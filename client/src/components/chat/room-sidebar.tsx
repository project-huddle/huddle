	...
	return (
		<aside className="hidden min-h-0 flex-col bg-(--panel) lg:flex">
			...
			<div className="flex-1 overflow-y-auto p-6">
									...
									!member.isOwner && (
										<div className="hidden gap-1 group-hover:flex">
											<button
												...
												className="rounded px-1.5 py-1 text-[10px] font-bold hover:bg-(--surface)"
				...
				<div className="rounded-[26px] bg-(--solid) p-5 text-(--on-solid)">
					<div className="flex items-center">
						<div className="grid size-10 place-items-center rounded-2xl bg-(--surface)/10">
					...
					<p className="mt-1 text-sm leading-relaxed text-(--on-solid)/55">
					...
					{!realtime.inCall ? (
						<button
							...
							className="mt-5 w-full rounded-2xl bg-(--brand) px-4 py-3 text-sm font-black text-(--ink) disabled:opacity-50"
						>
							{realtime.joining
								? "entrando..."
								: "entrar na chamada"}
						...
						<>
							...
							<button
								onClick={() => onOpenCall()}
								className="mt-3 w-full rounded-2xl bg-(--brand) px-4 py-3 text-sm font-black text-(--ink)"
							>
						...
						{realtime.peers.map(
							({ user: peer, sharing }) => (
								...
								>
									...
									{sharing && (
										<MonitorUp className="size-4 text-(--brand)" />
									)}
								</button>
							),
						)}
